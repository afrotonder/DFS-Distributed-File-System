...
class mds_db:
	...
	def RemoveFile(self, fname, fsize):  # ORR
		"""removes the inode attributes.  For this project the name of the
			file and its size.
		"""
		query = """remove from inode (fname, fsize) values ("%s", %s)""" % (fname, fsize)
		try:
			self.c.execute(query)
			return 1
		except:
