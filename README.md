...
* A metadata server: which will contain the metadata (inode) information of the files in your file system.  It will also keep registry of the data servers that are connected to the DFS.
* Data nodes: The data node will contain chunks (some blocks) of the file that you are storing in the DFS.
...
1. Write files to the DFS
    ...
    * Send the data blocks to each data node.  
    ...
    How to copy files to DFS: (copy to function)
    <pre>
...
- (10 pts) quality of the README
    + (10 pts) description of the programs with how to run them.
